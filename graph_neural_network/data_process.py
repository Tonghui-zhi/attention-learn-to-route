...
from nets.graph_encoder import HeteroGNN
...
def constract_graph_element_(instance, batch_size):
    """
    根据matrix造图
    """
...
def constract_graph_element_2(matrix_batch, capacity_batch, batch_size):
    """
    根据matrix造图
    """
    num_graph = matrix_batch.shape[0]
...
if __name__ == "__main__":
    # data = pd.read_pickle('../data/kf/kf20_kf_seed1234.pkl')[0: 10]
    #
    # dataset, dataloader = kefu_data(data)
    # for batch in dataloader:
    #     print(batch)
    ...
    batch_size = 30
    out_channels = 128
    dataset, dataloader = constract_graph_element_(data, batch_size)
    print(dataset)

    for batch in dataloader:
        model = HeteroGNN(hidden_channels=128, out_channels=128, num_layers=2, tar='item', heads=4)
        out = model(batch)
